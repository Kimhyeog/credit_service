# Phase 1 Q&A — HTTP vs HTTPS, middleware/, models/ 역할

> Phase 1 Step 1-1 "백엔드 디렉토리 구조 생성"에서 나온 질문과 답변 정리
...
> `routers/` — HTTP 요청을 받는 곳 (Controller 역할)이라고 했는데,
> HTTP로 하면 보안상 문제가 많지 않나?
> HTTPS로 하거나, Nginx로 HTTPS 설정을 통합해야 하는 거 아닌가?
> 뼈대 과정이라서 일단 HTTP로 세팅 후 나중에 HTTPS로 하려는 건가?
...
**결론:** 맞다 — 지금은 뼈대 단계라 HTTP로 개발하고, 배포 시 Nginx + HTTPS를 붙이는 방식이다.
...
> `middleware/` — 요청/응답을 가로채는 공통 처리
> `models/` — Pydantic 스키마 (요청/응답 형태 정의)
>
> 각각의 역할을 간단한 예시와 아스키코드로 설명해줄 수 있나?
> `models/`는 혹시 migration을 할 때 schema 업데이트나 생성에 필요한 코드가 들어있는 건가?

---

### middleware/ — 요청이 라우터에 도착하기 **전에** 가로채는 관문

#### 예시 1: 멱등성 미들웨어 (같은 주문 2번 방지)
...
- `models/schemas.py`는 **HTTP 레벨 계약** — "프론트가 보내야 할 JSON 모양"
- `prisma/schema.prisma`는 **DB 레벨 계약** — "테이블에 어떤 컬럼이 있는지"
- 둘은 **별개**이다
